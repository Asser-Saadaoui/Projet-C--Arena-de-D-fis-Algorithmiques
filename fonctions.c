...
void genererLabyrinthe(int map[TAILLE_LABY][TAILLE_LABY]) {
    ...
    // Assurer que le départ est en (1, 1)
    map[1][1] = 2;

    // Placer la sortie (3) aléatoirement sur un chemin (0), hors du départ
    int ex, ey;
    do {
...
int defi_Labyrinthe() {
    ...
    // Génère un nouveau labyrinthe à chaque appel
    genererLabyrinthe(map);

    int px_start = 1, py_start = 1;
    int ex = -1, ey = -1; // Position de la sortie

    // Pour gérer la boucle de saisie
    char chemin_saisi[200];
    int score_obtenu = 0;
    bool defi_termine = false;

    // --- AFFICHAGE INITIAL ET RECHERCHE DES POINTS DE DÉPART/ARRIVÉE ---
    nettoyerEcran();
    ...
    // =================================================================
    // NOUVELLE BOUCLE DE SAISIE DU CHEMIN
    // =================================================================
    while (!defi_termine) {
        ...
        if (strcmp(chemin_saisi, "quit") == 0 || strcmp(chemin_saisi, "quitter") == 0) {
            ...
            pause();
            return 0; // Sortie définitive si 'quit'
        }

        // --- ANALYSE ET EXÉCUTION DU CHEMIN ---
        int px = px_start;
        int py = py_start;
        int coups = 0;
        ...
        for (int i = 0; i < strlen(chemin_saisi); i++) {
            ...
            if(move=='n' || move=='N') nx--; // Nord (Haut)
                ...
                chemin_valide = false;
                break; // Sort de la boucle 'for' d'exécution du chemin
            }
        ...
        // --- GESTION DU RÉSULTAT DU CHEMIN SAISI ---

        if (!chemin_valide) {
            pause();
            continue; // Retourne en haut de la boucle 'while' pour une nouvelle saisie
        }

        // Le chemin est valide (n'a pas touché de mur ou caractère invalide)
        defi_termine = true; // Arrête la boucle 'while'
        ...
        if (map[px][py] == 3) {
            ...
            // Échec : Le chemin s'est terminé sans atteindre la sortie
...
int defi_TriUltime() {
    ...
    while(1) {
        ...
        // Affichage avec indices utilisateur (1 a 6)
        for(int i=0; i<6; i++) {
        ...
        if(estTrie) {
            ...
            // Scoring
            if (echanges <= 8) {
                printf(KGRN "Performance EXCELLENTE !\n" KNRM);
                return 100;
            } else if (echanges <= 12) {
                printf(KYEL "Performance BONNE.\n" KNRM);
                return 60;
            } else {
                return 30;
            }
        ...
        // PARSING ET VALIDATION DES INDICES
        int i1_user, i2_user;

        // S'assurer que l'utilisateur a entré exactement deux nombres
        ...
        // Plage valide (Indices utilisateur de 1 a 6)
        ...
        // CONVERSION DES INDICES (Utilisateur 1..6 -> Tableau C 0..5)
        int i1_tab = i1_user - 1;
        int i2_tab = i2_user - 1;

        // EXECUTION DE L'ÉCHANGE
        int tmp = tab[i1_tab];
...
int defi_VoyageurDeCommerce() {
    ...
    while (!defi_termine) {
        ...
        // 2. PARSING ET VÉRIFICATION DES CARACTÈRES/UNICITÉ
        for(int i=0; i<NB_VILLES; i++) {
